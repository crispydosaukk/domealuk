    ...
    return (
      <div className="bg-white rounded-2xl border border-border p-16 text-center shadow-sm">
        ...
        <Link
          ...
          className="inline-flex bg-primary text-white font-700 px-6 py-3 rounded-xl hover:bg-red-900 transition-all active:scale-95"
        >
          Sign In
        </Link>
      </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm">
          <p className="text-xs text-muted-foreground font-600 uppercase tracking-wide mb-1">
            Total Orders
          </p>
        ...
        <div className="bg-red-50 rounded-2xl border border-red-200 p-5 shadow-sm">
          <p className="text-xs text-primary font-600 uppercase tracking-wide mb-1">
            Notifications
          </p>
          <div className="flex items-center gap-2">
            <Bell size={18} className="text-primary" />
            <p className="text-sm font-700 text-foreground">Email On</p>
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-border p-16 text-center shadow-sm">
          ...
          <Link
            href="/menu"
            className="inline-flex items-center gap-2 bg-primary text-white font-700 px-5 py-3 rounded-xl hover:bg-red-900 transition-all active:scale-95 text-sm"
          >
            Browse Menu
          </Link>
        </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border-2 transition-all duration-200 overflow-hidden ${
                  isExpanded
                    ? 'border-primary shadow-md shadow-red-50'
              ...
              >
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${cfg.bg}`}
                  >
                  ...
                  <div className="flex-1 min-w-0 w-full">
                    <div className="flex flex-wrap items-center gap-2 mb-1 w-full">
                      ...
                      <span
                        className={`text-xs font-700 px-2.5 py-1 rounded-md ${cfg.bg} ${cfg.color}`}
                      >
                        {cfg.label}
                      </span>
                      {order.paymentMethod && (
                        <span
                          className={`text-[10px] font-700 px-2 py-0.5 rounded-md border ${
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-0 border-border/50 shrink-0">
                    <p className="font-800 text-lg text-foreground tabular-nums">
                ...
                {isExpanded && (
                  <div className="border-t border-border px-5 pb-5 pt-4 bg-muted/10">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <div>
                        {order.allergiesInfo && (
                          <div className="bg-red-50 p-4 rounded-xl border border-red-200 shadow-sm mb-5">
                            ...
                            <p className="text-sm font-600 text-red-900">{order.allergiesInfo}</p>
                          </div>
                        )}
                        <p className="text-[10px] font-800 text-muted-foreground uppercase tracking-wider mb-3">
                          Items Ordered
                        </p>
                          ...
                          {order.items?.map((item: any, idx: number) => {
                            const extraPriceTotal =
                              item.subItems?.reduce(
                                (sum: number, sub: any) => sum + (sub.price || 0),
                                0
                              ) || 0;
                              ...
                              >
                                <div className="flex justify-between text-sm">
                                  <span className="text-foreground font-500">
                                    <span className="font-700">{item.qty}x</span> {item.name}
                                  </span>
                                  ...
                                  <div className="pl-6 space-y-0.5">
                                    {item.subItems.map((sub: any, sIdx: number) => (
                                      ...
                                      >
                                        ...
                                        {sub.price > 0 && (
                                          <span>+£{(sub.price * item.qty).toFixed(2)}</span>
                                        )}
                                      </div>
                                    ))}
                                  </div>
                              ...
                              <span className="tabular-nums">
                                ...
                                {(
                                  ...
                                  order.total +
                                    ...
                                    (order.dabbaFeeApplied ? order.dabbaFee || 12.0 : 0)
                                ).toFixed(2)}
                              </span>
                            </div>
                            {order.studentDiscountApplied > 0 && (
                              <div className="flex justify-between font-700 text-xs text-[#C39B54]">
                                <span>
                                  🎓 Student Discount ({order.studentDiscountPercent || 0}%)
                                </span>
                                <span className="tabular-nums">
                                  -£{order.studentDiscountApplied.toFixed(2)}
                                </span>
                              </div>
                            )}
                            {order.discountApplied > 0 && (
                            ...
                            {order.walletApplied > 0 && (
                              <div className="flex justify-between font-700 text-xs text-green-700">
                                <span className="flex items-center gap-1">
                                  <Wallet size={12} /> Wallet Applied
                                </span>
                                <span className="tabular-nums">
                                  -£{order.walletApplied.toFixed(2)}
                                </span>
                              </div>
                            )}
                        ...
                        <p className="text-[10px] font-800 text-muted-foreground uppercase tracking-wider mt-5 mb-3">
                          Payment Details
                        </p>
                          ...
                          {order.stripeSubscriptionId && (
                            <div className="flex items-start gap-3 border-t border-border/50 pt-2.5">
                              <div className="flex-1 min-w-0">
                                <p className="font-700 text-foreground text-xs">
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground mt-0.5 text-xs">
                                ...
                                <br />
                                Slot:{' '}
                                {order.deliverySlot === 'slot-1'
                                  ? 'Morning (7:30 AM - 8:30 AM)'
                                  : order.deliverySlot === 'slot-2'
                                    ? 'Afternoon (12:00 PM - 1:00 PM)'
                                    : order.deliverySlot === 'slot-3'
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground leading-relaxed mb-6">
              Are you sure you want to cancel your tiffin subscription? This will stop all upcoming
              scheduled tiffin deliveries for this order.
            </p>
