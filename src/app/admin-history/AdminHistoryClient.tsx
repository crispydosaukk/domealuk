        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted border-b border-border">
              <tr>
                <th className="text-left px-5 py-3 text-xs font-700 text-muted-foreground uppercase">Order ID</th>
                <th className="text-left px-5 py-3 text-xs font-700 text-muted-foreground uppercase">Customer</th>
                <th className="text-left px-5 py-3 text-xs font-700 text-muted-foreground uppercase">Items</th>
                <th className="text-left px-5 py-3 text-xs font-700 text-muted-foreground uppercase">Date Delivered</th>
                <th className="text-left px-5 py-3 text-xs font-700 text-muted-foreground uppercase">Total</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {loading ? (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-muted-foreground">Loading history...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-12 text-center text-muted-foreground">No completed orders found.</td></tr>
              ) : filtered.map(order => (
                <tr key={order.id} className="hover:bg-muted/40 transition-colors">
                  ...
                  <td className="px-5 py-4 text-muted-foreground whitespace-nowrap text-xs">
                    {order.createdAt?.toDate ? order.createdAt.toDate().toLocaleDateString('en-GB') : 'Unknown'}
                  </td>
                  <td className="px-5 py-4 font-700 tabular-nums text-foreground">£{(order.total || 0).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
